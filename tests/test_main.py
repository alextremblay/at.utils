...
def test_get_config_file(mock_si_utils_paths: dev_utils.SIUtilsPaths):
    ...
    mock_si_utils_paths.set_config_toml("someapp", " ")
...
@pytest.mark.forked
def test_get_config_obj_raises(mock_si_utils_paths: dev_utils.SIUtilsPaths):
    mock_si_utils_paths.set_config_yaml("test", " ")
    with pytest.raises(Exception) as exc_info:
...
@pytest.mark.forked
def test_get_config_obj_ini(mock_si_utils_paths: dev_utils.SIUtilsPaths):
    mock_si_utils_paths.set_config_ini(
        "test",
        """
        [DEFAULT]
        key1 = val1
...
@pytest.mark.forked
def test_get_config_obj_json(mock_si_utils_paths: dev_utils.SIUtilsPaths):
    mock_si_utils_paths.set_config_json(
        "test",
        """
        {
            "key1": "val1", 
...
@pytest.mark.forked
def test_get_config_obj_toml(mock_si_utils_paths: dev_utils.SIUtilsPaths):
    mock_si_utils_paths.set_config_toml(
        "test",
        """
        key1 = 'val1'
        key2 = 'val2'
...
def test_get_cache_dir(mock_si_utils_paths: dev_utils.SIUtilsPaths):
    mock_si_utils_paths.site_cache.joinpath("test").mkdir()
...
Skipped by default, but should be run periodiucally to ensure Timeit API hasn't diverged from expectation""")
def test_timeit():
